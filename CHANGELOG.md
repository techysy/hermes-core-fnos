...
## 0.4.7.14-test (2026-08-05)

> 测试包 — 整体配色对齐飞牛官方。基于 v0.4.7.13。

### 变更 / Changed
- **整体配色对齐飞牛官方视觉** — 主色改为 `#1677ff`（飞牛蓝）、浅灰背景 `#f0f2f5`、更规范的边框/文字灰阶
- **导航选中项** — 浅蓝背景 + 蓝色文字 + 左侧蓝色竖向指示条（hover 用白色背景区分）
...
### 修复 / Fixed
- **移动端汉堡菜单下移** — `.hamburger` 同样受全局 `button { margin-top:16px }` 影响下沉。`.hamburger` 加 `margin-top:0`，与语言/主题按钮对齐

## 0.4.7.11-test (2026-08-05)

> 测试包 — 顶栏按钮对齐。基于 v0.4.7.10。

### 修复 / Fixed
- **顶栏语言/主题按钮下移** — 全局 `button { margin-top:16px }` 导致顶栏按钮额外下沉 16px，比侧边栏「聊天」项低。`.icon-btn` 加 `margin-top:0`，按钮与侧边栏第一项对齐

## 0.4.7.10-test (2026-08-05)

> 测试包 — 移除左上角品牌区。基于 v0.4.7.9。

### 变更 / Changed
- **移除左上角品牌区** — 侧边栏顶部的「🔧 Hermes Core / 本地内核」品牌块已移除（fnOS 应用自带标题栏显示应用名，品牌区重复）。侧边栏现在直接从「聊天」导航项开始

## 0.4.7.9-test (2026-08-05)

> 测试包 — 修复移动端布局异常。基于 v0.4.7.8。

### 修复 / Fixed
- **移动端主内容被挤压成几十px** — 根因：移除消息平台底部状态(`{MSG_STATUS}`)时多留了一个 `</div>`，导致 `.main` 在消息平台面板后提前闭合，`panel-providers` 和 `meta` 脱离 `.main` 变成 `.layout` 的 flex 项，挤压 `.main` 宽度（移动端只剩 ~66px）。已删除多余 `</div>`，布局恢复正常（移动端 `.main` 占满宽度）

## 0.4.7.8-test (2026-08-05)

> 测试包 — 修复图标重复与版本号显示。基于 v0.4.7.7。

### 修复 / Fixed
- **重启菜单图标重复（🔄🔄）** — 根因：i18n `restart` 值含 🔄，与菜单里的 icon span 🔄 叠加，导致显示两个图标。去掉 i18n 值里的 🔄，只保留 icon span 的一个
- **版本号窄屏显示** — `.meta` 版本信息行在窄屏下文字被截断/显示异常（"本地内核"的"本"被截断），加 `word-break:break-all` 防止裁剪

## 0.4.7.7-test (2026-08-05)

> 测试包 — 进一步 UI 精简。基于 v0.4.7.6。

### 修复 / Fixed
- **右上角语言按钮** — 去掉「EN/中文」文字，只保留 🌐 图标（界面切换后语言显而易见，无需文字标注）
- **移除消息平台底部重复状态** — 底部「飞书/微信/QQ/钉钉 已配置」汇总与各卡片自带的状态重复，且状态页已有独立「消息网关」卡片，故移除底部汇总

## 0.4.7.6-test (2026-08-05)

> 测试包 — UI 优化（5 项修复）。基于 v0.4.7.5。

### 修复 / Fixed
- **顶栏布局调整** — 语言切换/主题 放回顶栏（外部图标按钮），⋮ 菜单改为「更新日志 + 重启内核」，去除重启图标重复
- **消息平台卡片交互优化** — 卡片加高；配置改为绝对定位覆盖层（同供应商卡片交互，不再撑开卡片容器），点击卡片弹出覆盖层编辑，保存后自动收起
- **保存按钮缩小** — 配置页、默认模型等分页的保存按钮改为紧凑尺寸（不再全宽撑满）
- **配置提示重叠优化** — 绿色成功提示与卡片容器不再重叠（调整间距 + 保存后自动关闭覆盖层）
- **飞书卡片** — 「扫码授权」改为「去飞书开放平台创建应用」引导链接（飞书无 Hermes 原生扫码，需手动创建应用取 App ID/Secret）

## 0.4.7.5-test (2026-08-05)

> 测试包 — 顶栏统一菜单 + 消息平台卡片网格。基于 v0.4.7.4。

### 变更 / Changed
- **顶栏统一为 ⋮ 菜单** — 语言切换/主题/重启内核 收进右上角一个 ⋮ 下拉菜单，移除散落的图标，界面更干净
- **消息平台改为平台卡片网格** — 参考供应商页卡片样式，内置 4 个平台：飞书/微信/QQ/钉钉，点击卡片展开配置字段，卡片显示已配置/未配置状态
  - 新增 QQ（App ID/Secret）和钉钉（Client ID/Secret）平台配置字段
  - 微信卡片保留扫码登录入口

## 0.4.7.4-test (2026-08-05)

> 测试包 — 优化重启内核入口。基于 v0.4.7.3。

### 变更 / Changed
- **重启内核统一入口** — 重启按钮移到右上角 🔄（所有面板可见），移除配置页/消息平台页内的重复重启按钮，避免入口多而乱
- **修复重启"看起来无效"** — 状态服务重启时会被杀掉导致浏览器无响应；重启后自动刷新页面（5秒），让重启操作有明确反馈
...
### 新增 / Added
- **📡 消息平台设置面板** — 侧边栏新增「消息平台」，配置飞书（App ID/Secret/验证 Token/加密 Key）和微信（账号 ID/Token），保存后重启内核生效；面板显示飞书/微信连接状态（已配置/未配置）
  - **微信用内置适配器** — 配置 WEIXIN_TOKEN + WEIXIN_ACCOUNT_ID 后网关自动启用（无需插件），cmd/main 已导出 env
  - **飞书自动装插件** — 配置飞书凭据后，cmd/main 的 setup_plugins 自动安装 hermes-lark-streaming 插件并启用
